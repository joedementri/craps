...
function renderBoardAmounts() {
  els.zoneAmounts.forEach((el) => {
    ...
    const zoneEl = el.closest(".bet-zone");
    if (!zoneEl) return;
...
function renderPointPuck() {
  ...
  const zone = document.querySelector(`.place-zone[data-bet="place${state.point}"]`);
  if (!zone) return;
...
function processRoll(d1, d2) {
  ...
  if (!placeWorking && phaseBefore === "comeout" && getPlaceBetTotal() > 0) {
    events.push({ text: "Place bets were OFF for the come out roll.", type: "neutral" });
  }
...
function attachEvents() {
  ...
  els.betsOffToggle.addEventListener("click", () => {
    ...
    note(`Place bets are now ${state.betsWorking ? "ON" : "OFF"} for come out rolls.`);
  });
